...
The Mean Corpuscolar Value (MCV) was used in order to identify primarily genetic diseases ($\beta$-thalassemia minor) and carential states (iron and vitamin $B_{12}$ deficiencies). This was ANOTHER little prick, but we managed with \cite{MCVferritincutoff}. It depends on various factors:
...
\footnotetext[1]{All ferritin values are expressed in $\si{\gram/\litre}$}
\footnotetext[2]{All MCV values are expressed in $\si{\gram/\litre}$}
...
%Completa elenco puntato
%Sistema Unità di misura!
%Vedi se devi dire altro ma non credo
