...
%QUALI PROBLEMATICHE HANNO DALLA LETTERATURA? VEDI ARTICOLI 2, 5, 8, 14
...
Several systematic reviews and eminent periodicals focused on infectious disease risk among internationally adopted children, while data regarding prevalence and spectrum of non-infectious conditions, including special needs, are poor (see \cite{notonlyinfectious} and \cite{nonsoloinfezioni}). Many studies (a.e. see \cite{notonlyinfectious} and \cite{obringer}) have assessed prevalence of all infections, ranging between 35 and 42\% for all infections, ~40\% for parasitic ones (see \cite{GNLBIexp} and \cite{sollai}), and between 5 and 19\% for tuberculosis (see \cite{sollai} and \cite{ampofo}). 

VITAMIN D is not in reviews :( %TODO

INTEGRA GNLBI 1 2 3 e EXP %TODO

The sanitary surveillance on internationally adopted children, both clinical and test-based, in Italy is based in 20 centers, one per region. In Friuli-Venezia Giulia this center is placed in \textit{San Vito al Tagliamento}, where the data for this paper came from. For more a more in depth overview on the work they do on a day-to-day basis, see the Introduction to Chapter \ref{chap:materialsandmethods} at page \pageref{chap:materialsandmethods}.
